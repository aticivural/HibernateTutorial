...
public class CriteriaQueryTest {
    ...
    public void testORQueryPlusGaming() {
        Criteria crit = session.createCriteria(Product.class);
        Criterion priceLessThan = Restrictions.gt("price", 20.0);//2
        Criterion mouse = Restrictions.ilike("description", "mouse", MatchMode.ANYWHERE);//3
        LogicalExpression orExp = Restrictions.or(priceLessThan, mouse);
        crit.add(orExp);
    ...
    public void testCriteriaAssociation() {
        ...
        crit.list().forEach(System.out::println);
    }
